...
public:
	...
	const BigInt operator - (const BigInt &) const;
	const BigInt operator * (const BigInt &) const;
	const BigInt operator / (const LL &) const;
	const LL operator % (const LL &) const;
...
const BigInt BigInt::operator / (const LL & rhs) const {
	LL temp = 0;
	BigInt x = (*this), res;
...
const LL BigInt::operator % (const LL & y) const {
	LL res = 0;
